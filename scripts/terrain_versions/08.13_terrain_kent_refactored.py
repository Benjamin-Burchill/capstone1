"""
08.13_terrain_kent_refactored.py - REFACTORED MODULAR Kent terrain
Massive scale: 59,000 x 59,000 units with clean class structure
Features: Separated concerns - TerrainLogger, CameraController, NoiseGenerator, LoadingManager
"""
        ...
        for j in range(resolution):
            for i in range(resolution):
                if coastline_mask[j, i] > 0:  # Land areas
                    ...
                    # Multi-scale terrain with height constraints
                    continental_base = self.noise.fractal_brownian_motion(wx, wy, octaves=3, frequency=0.00001, amplitude=400, seed=42)
                    continental_base = max(0, continental_base * 0.7)
                    
                    regional_hills = self.noise.fractal_brownian_motion(wx, wy, octaves=4, frequency=0.00005, amplitude=150, seed=123)
                    local_terrain = self.noise.fractal_brownian_motion(wx, wy, octaves=5, frequency=0.0002, amplitude=40, seed=456)
                    fine_detail = self.noise.fractal_brownian_motion(wx, wy, octaves=4, frequency=0.001, amplitude=10, seed=789)
                    micro_detail = self.noise.fractal_brownian_motion(wx, wy, octaves=3, frequency=0.005, amplitude=2.5, seed=101112)
...
class RefactoredKentTerrain(ShowBase):
    """Main terrain application with clean modular structure"""
    ...
    def __init__(self):
        ...
        props.setTitle("Refactored Kent Terrain - 3,500 km² (MODULAR DESIGN)")
...
if __name__ == "__main__":
    print("\n=== REFACTORED MODULAR Kent Terrain ===")
    print("Scale: 59km x 59km (3,500 km²)")
    print("🏗️ CLEAN ARCHITECTURE:")
    print("  • TerrainLogger: Centralized logging system")
    print("  • CameraController: All camera movement and controls")
    print("  • NoiseGenerator: Noise algorithms (static methods)")
    print("  • LoadingManager: Progress tracking and UI")
    print("  • TerrainGenerator: Terrain creation algorithms")
    print("  • RefactoredKentTerrain: Main coordinator class")
    print("🎯 BENEFITS:")
    print("  • Single Responsibility Principle")
    print("  • Better code readability and maintenance")
    print("  • Easier testing and debugging")
    print("  • Modular components can be reused")
    print("\nStarting REFACTORED terrain generation...")
    app = RefactoredKentTerrain()
    app.run()
